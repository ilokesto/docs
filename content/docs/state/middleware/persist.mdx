...
`pipe.use(persist(...)).use(debounce(...))` is rejected with `MIDDLEWARE_ORDER` because it would skip persistence for deferred commits. Pipe keeps the declared order and never rearranges it.
