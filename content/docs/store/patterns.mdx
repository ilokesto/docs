...
Use this page when you are writing app code with `@ilokesto/store`. It collects small recipes for creating stores, updating them safely, and keeping state easy to test.

If you are writing a framework adapter or library on top of `Store`, read [Building on Store](./building-on-store) instead. For exact replacement, subscription, and bailout rules, read [Advanced Semantics](./advanced-semantics). For the full method list, read the [API Reference](./api).
...
- Mutating objects from `getState()` instead of replacing state with a new value.
- Forgetting that object updates replace the whole state and do not merge fields.
- Calling `setState` from a subscriber without a guard, which can create a synchronous loop.
- Sharing one module-level store across tests that expect isolated state.
