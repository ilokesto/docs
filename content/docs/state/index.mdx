...
If you are new to the package, start with [Quick Start](./quick-start).

- Read [Mental Model](./mental-model) if you want to understand why the store lives outside React.
- Read [Selectors](./selectors) if the selector/writer split is the part that still feels strange.
- Read [Existing Store](./existing-store) if you already own a vanilla `Store` and only need the React binding layer.
