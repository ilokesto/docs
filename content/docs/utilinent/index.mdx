...
- Read [Core Concepts](./core-concepts) if you want to understand how the package is organized.
- Go to [Conditional Rendering](./conditional) first if your main use case is JSX branching.
- Go to [Collection Helpers](./collection) if the main pain point is `map()` plus fallback handling.
- Go to [Composition](./composition) if the main pain point is `asChild`, slotting, or wrapperless composition.
- Go to [Async & Visibility](./async) if loading, viewport entry, or deferred rendering is the problem.
