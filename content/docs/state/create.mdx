...
If the second form is your real use case, read [Existing Store](./existing-store) next. That page explains when `create(store)` is the right boundary and what does not change when you bind React onto an already-owned store.
...
- Read [Selectors](./selectors) if you want the subscription and memoization model in detail.
- Read [Accessors](./accessors) if you plan to move actions outside components.
- Read [Existing Store](./existing-store) if your app already owns a vanilla `Store` and only needs the React hook layer.
