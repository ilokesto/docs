...
Registration errors and notification errors have different boundaries:
...
- **Notification (a later `setState()` call).** The selector runs, then the equality function runs, then the listener runs only when the equality function reports a change. All three run synchronously inside the notification cycle. Failures are collected without skipping other active listeners or queued commits, then the outermost delivery throws an `AggregateError` containing the original failures.

Keep the selector, equality function, and listener small, or catch expected errors inside the listener.
## Subscription ownership in the next major

Registering the same callback twice creates independent subscriptions. Unsubscribe immediately cancels pending delivery for that registration. Reentrant updates commit immediately but notifications drain in FIFO order. Selectors receive each committed snapshot; `getState()` always returns the latest value.
