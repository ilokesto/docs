...
- Read [Selectors](./selectors) if you want the exact subscription and memoization model behind the tuple you just used.
- Read [Accessors](./accessors) if you plan to define actions outside components.
- Read [Existing Store](./existing-store) if your app already owns a vanilla `Store` and only needs the hook layer.
